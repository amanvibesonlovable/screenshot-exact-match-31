...
function ReportHeader({ name, period }: { name: string; period?: string }) {
  return (
    <header className="mb-6 border-b border-border pb-4">
      <p className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Pulse · {name}</p>
      <p className="mt-1 text-xs text-muted-foreground">{period ? `Period: ${period} · ` : ""}Generated {new Date().toLocaleString()}</p>
    </header>
  );
}
function ReportFooter() {
  return <footer className="mt-8 border-t border-border pt-3 text-[10px] text-muted-foreground">Generated by Pulse — Confidential</footer>;
}
